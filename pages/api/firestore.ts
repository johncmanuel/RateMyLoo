...
export default async function handler(
  ...
  switch (method) {
    ...
    case "POST":
      try {
        const docRef = await firestore.collection(collection).doc(userId);
        ...
        if (collection == "history") {
          if (!(getUserIdFromImageUrl in existingData)) {
            ...
            const arrImgUrls: string[] = existingData[getUserIdFromImageUrl];
            if (!arrImgUrls.includes(parsedBody)) arrImgUrls.push(parsedBody);
          }
        }
