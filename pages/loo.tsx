...
const Loo = ({ favoriteColor, favoriteAnimal, email }: any) => {
	const user = useUser();
	return (
		<div>
			...
			<div style={styles.content}>
				<div style={styles.infoTextContainer}>
					...
					<p>
						...
						server-side redirect (307) to the login page if the auth
						cookies are not set.
					</p>
					<p>Your favorite color is: {favoriteColor}</p>
					<p>
						Your favorite animal is {favoriteAnimal}. Please send
						lots of photos of {favoriteAnimal} to {email}.
					</p>
				</div>
			</div>
		</div>
...
export const getServerSideProps = withUserTokenSSR({
	...
	const endpointTwo = getAbsoluteURL("/api/cookies", req);
